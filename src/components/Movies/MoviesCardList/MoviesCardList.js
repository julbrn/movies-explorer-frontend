...
import { ERR_MESSAGE, DESKTOP_WIDTH, TABLET_WIDTH, MOBILE_WIDTH, DESKTOP_CARD_QUANTITY, TABLET_CARD_QUANTITY, MOBILE_CARD_QUANTITY } from '../../../utils/constants';
          ...
          {isNoMatches ? (
            ...
            serverErrorMessage ? (<span className='movies__error'>{serverErrorMessage}</span>) : (
              <>
                ...
                {((location === '/movies') && !isButtonHidden) ? (<button type="button" className="movies__btn" aria-label='Показать ещё'>Ещё</button>) : null}
              </>))
          }
        </>
      )}
